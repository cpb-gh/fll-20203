...
async def craft_creator():
    ...
    await gyro_straight(degrees= 1000, forward= True)
...
async def experimental_dragon():
    ...
    await gyro_straight(degrees= 420, forward=True)

async def run_across():
    light_matrix.write("going across!")
    await gyro_straight(degrees=1100, forward=True)
    await turn(degrees=40, direction="right", speed= 100)
    await gyro_straight(degrees = 200)
    await turn(degrees=40, direction="left", speed= 100)
    await gyro_straight(degrees=0)
    await gyro_straight(degrees = 2000)

async def music_maker():
    await gyro_straight(degrees=1200)
